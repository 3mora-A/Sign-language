#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""أدوات استخراج خصائص المشاعر من الصور والفيديو.

الهدف من هذا الملف هو إعادة إنتاج نفس نوع الـ features التي تم التدريب عليها
داخل `X.npy` و `y.npy`، حتى يكون الاستدلال وقت التشغيل متوافقًا مع التدريب.

المسار العام هنا هو:
- تجهيز الصورة أو اختيار frame مناسب من الفيديو
- تطبيق بعض المعالجات البصرية عند الحاجة
- تمرير الصورة إلى EfficientNet-B0 بدون طبقة التصنيف
- الحصول على vector ثابت الطول يصلح للإدخال إلى SVM
"""
