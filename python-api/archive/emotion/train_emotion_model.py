...
DATA_DIR = PROJECT_DIR / "data" / "emotion_vectors"
...
class Config:
    ...
    LABEL_MAP_PATH = os.path.join(DATA_DIR, "emotion_label_map.json")
    TEST_SIZE = 0.2
    RANDOM_SEED = 42
    USE_PCA = False
    PCA_COMPONENTS = 256
...
def main():
    ...
    labels_path = os.path.join(ARTIFACTS_MODELS_DIR, config.LABELS_FILENAME)
    pipeline_model_path = os.path.join(ARTIFACTS_MODELS_DIR, config.PIPELINE_MODEL_FILENAME)
    encoder_path = os.path.join(ARTIFACTS_MODELS_DIR, config.ENCODER_FILENAME)
    report_path = os.path.join(ARTIFACTS_MODELS_DIR, config.REPORT_FILENAME)
    split_path = os.path.join(ARTIFACTS_MODELS_DIR, config.SPLIT_FILENAME)
    ...
    for legacy_filename in ("emotion_svm.pkl", "emotion_scaler.pkl", "emotion_pca.pkl"):
        legacy_path = os.path.join(ARTIFACTS_MODELS_DIR, legacy_filename)
